    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-gray-900">{activeChatbot.name}</h2>
        ...
        <div className="flex items-center space-x-3">
          <span className={`flex items-center space-x-2 px-3 py-1 rounded-full text-sm font-medium ${
            activeChatbot.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'
          }`}>
            {activeChatbot.isActive ? <Play className="w-4 h-4" /> : <Pause className="w-4 h-4" />}
            <span>{activeChatbot.isActive ? 'Active' : 'Paused'}</span>
          </span>
          
          <button
            onClick={() => handleUpdateField('isActive', !activeChatbot.isActive)}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <div className="flex items-center space-x-2">
              <span className="text-sm text-gray-600">{activeChatbot.lastTrained}</span>
              <button
                onClick={handleTrain}
              ...
              >
                <RefreshCw className={`w-4 h-4 ${activeChatbot.isTraining ? 'animate-spin' : ''}`} />
                <span>{activeChatbot.isTraining ? 'Training...' : 'Retrain'}</span>
              </button>
            </div>
          </div>
          <div>
